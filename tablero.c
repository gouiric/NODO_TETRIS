...
//Reemplazo dibujarRectangulo por dibujar_rect local
// void dibujar_rect(int x, int y, int ancho, int alto, uint8_t color);
...
void inicialiar_tablero(Tablero* t)
{
    ...
    t->bloque_original = bloque_memoria;
...
void dibujar_tablero(Tablero* t, uint16_t offsetX, uint16_t offsetY)
{
    for(int y = 0; y < TABLERO_FILAS; y++)
    {
        for(int x= 0; x < TABLERO_COLS; x++)
        {
            ...
            if(t->matriz[y][x] != 0)
            {
                dibujar_rect(offsetX + (x *TAMANIO_BLOQUE), offsetY + (y * TAMANIO_BLOQUE),TAMANIO_BLOQUE -1 , TAMANIO_BLOQUE -1, 4);
            }

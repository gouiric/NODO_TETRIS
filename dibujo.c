...
void dibujar_texto(char* texto, float porcXI, float porcYI, int escala, int col)
{
    ...
    while(*texto)
    {
        ...
        if(actual == ESPACIO)
        {   // Definimos que un espacio en blanco mida 3 p�xeles a la escala actual
...
{
    // Recorremos las columnas desde la derecha (�ndice 7 bajando al 0)
    for (int col = 7; col >= 0; col--) {
        // Revisamos las 8 filas de esa columna
        for (int fila = 0; fila < 8; fila++) {
            if (letra[fila][col] == 1) {
                // Encontramos el borde de la letra
                // Le sumamos 1 porque los �ndices van de 0 a 7.
                return col + 1;
            }
        }
    }
